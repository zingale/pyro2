subroutine multid_limit(qx, qy, ng, dx, dy, &
                        a, ld_ax, ld_ay)
  ...
  double precision, intent(out) :: ld_ax(0:qx-1, 0:qy-1)
  double precision, intent(out) :: ld_ay(0:qx-1, 0:qy-1)

!f2py depend(qx, qy) :: a
!f2py depend(qx, qy) :: ld_ax, ld_ay
!f2py intent(in) :: a
!f2py intent(out) :: ld_ax, ld_ay
  ...
  !    a(x,y) = A_xy (x-x_i)(y-y_i) + A_x (x-x_i) + A_y (y-y_i) + A_avg
  ...
  do j = jlo-2, jhi+2
     do i = ilo-2, ihi+2
        ...
        ss_temp(ill) = a(i,j) - 0.5d0*dx*A_x - 0.5d0*dy*A_y + 0.25d0*dx*dy*A_xy
        ss_temp(ilh) = a(i,j) - 0.5d0*dx*A_x + 0.5d0*dy*A_y - 0.25d0*dx*dy*A_xy
        ss_temp(irl) = a(i,j) + 0.5d0*dx*A_x - 0.5d0*dy*A_y - 0.25d0*dx*dy*A_xy
        ss_temp(irh) = a(i,j) + 0.5d0*dx*A_x + 0.5d0*dy*A_y + 0.25d0*dx*dy*A_xy
